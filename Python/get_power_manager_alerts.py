            ...
            if alerts_metrics_response.status_code != 201 & alerts_metrics_response.status_code != 200:
                if 'error' in alerts_metrics_json_data:
                    ...
                    if '@Message.ExtendedInfo' not in error_content:
                        ...
                        extended_error_content = error_content['@Message.ExtendedInfo']
                        print(
                            "Unable to retrieve Power Manager Metrics Alerts from %s. See below ExtendedInfo for more information" % ip_address)
                        print(extended_error_content[0]['Message'])
                else:
                    print("Unable to retrieve Power Manager Metrics Alerts from %s" % ip_address)
            else:
                ...
                if alerts_metrics_count <= 0:
                    ...
                    if alerts_metrics_content:
                        ...
                        for alerts_metrics_elem in alerts_metrics_content["value"]:
                            alerts_metrics_data = [alerts_metrics_elem["SeverityName"],
                                                   alerts_metrics_elem["AlertDeviceName"],
                                                   alerts_metrics_elem["TimeStamp"],
                                                   alerts_metrics_elem["CategoryName"],
                                                   alerts_metrics_elem["SubCategoryName"],
                                                   alerts_metrics_elem["AlertMessageId"],
                                                   alerts_metrics_elem["Message"]]
            ...
            if alerts_power_config_response.status_code != 201 & alerts_power_config_response.status_code != 200:
                if 'error' in alerts_power_config_json_data:
                    ...
                    if '@Message.ExtendedInfo' not in error_content:
                        ...
                        extended_error_content = error_content['@Message.ExtendedInfo']
                        print(
                            "Unable to retrieve Power Manager - Power Configuration Alerts from %s. See below ExtendedInfo for more information" % ip_address)
                        print(extended_error_content[0]['Message'])
                else:
                    print("Unable to retrieve Power Manager - Power Configuration Alerts from %s" % ip_address)
            else:
                ...
                if alerts_power_config_count <= 0:
                    ...
                    if alerts_power_config_content:
                        ...
                        for alerts_power_config_elem in alerts_power_config_content["value"]:
                            alerts_power_config_data = [alerts_power_config_elem["SeverityName"],
                                                        alerts_power_config_elem["AlertDeviceName"],
                                                        alerts_power_config_elem["TimeStamp"],
                                                        alerts_power_config_elem["CategoryName"],
                                                        alerts_power_config_elem["SubCategoryName"],
                                                        alerts_power_config_elem["AlertMessageId"],
                                                        alerts_power_config_elem["Message"]]
