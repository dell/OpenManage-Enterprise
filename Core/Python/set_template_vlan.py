...
def get_vlan_payload(base_uri, headers, template_id, nic_identifier, untag_dict, tagged_dict):
    ...
    # VlanAttributes
    port_id_map, port_untagged_map, port_tagged_map = get_template_vlan_info(base_uri, headers, nic_identifier,
                                                                             template_id)
...
def template_assign_vlan(base_uri, headers, template_id, network_card, untag_dict, tagged_dict):
    ...
    try:
        ...
        payload = get_vlan_payload(base_uri, headers, template_id, network_card, ast.literal_eval(untag_dict),
                                   ast.literal_eval(tagged_dict))
    ...
    try:
        if ARGS.name:
            ...
            for template in templates:
                template_assign_vlan(base_uri, headers, template["Id"], ARGS.network_card, ARGS.untagged_vlans,
                                     ARGS.tagged_vlans)
    except Exception as e:
        print(traceback.format_exc())
    finally:
